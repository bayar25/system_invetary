...
import template from "./templates/bill";
...
export default class bill {
  ...
  fill() {
    return new Promise((resolve, reject) => {
      ...
      dom.querySelector(".total").innerText = this.formatMoney(this.sale.total);
      ...
      dom.querySelector(".total2").innerText = this.formatMoney(
        this.sale.total
      );
      ...
      this.sale.listDt.forEach((element, index) => {
        ...
        cell3.innerHTML = this.formatMoney(element.price);
        cell3.className = "cellPrice";
        let cell4 = row.insertCell(3);
        cell4.innerHTML = this.formatMoney(element.total);
        ...
        t2Cell3.innerHTML = this.formatMoney(element.price);
        t2Cell3.className = "cellPrice";
        let t2Cell4 = row2.insertCell(3);
        t2Cell4.innerHTML = this.formatMoney(element.total);
  ...
  print() {
    return new Promise((resolve, reject) => {
      ...
      print
        .run(this.urlPDF, `Factura ${this.code} Impresa`)
        .then(resp => {
          resolve(resp);
        })
        .catch(err => {
